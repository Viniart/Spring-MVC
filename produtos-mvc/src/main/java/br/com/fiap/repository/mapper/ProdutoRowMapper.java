...
public class ProdutoRowMapper implements RowMapper<ProdutoModel> {
	...
	public ProdutoModel mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		ProdutoModel produto = new BeanPropertyRowMapper
				<>(ProdutoModel.class).mapRow(rs, rowNum);
		CategoriaModel categoria = new BeanPropertyRowMapper
				<>(CategoriaModel.class).mapRow(rs, rowNum);
